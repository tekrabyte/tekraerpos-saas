...
export default function Promo() {
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            <PageHeader title="Promo & Voucher" subtitle="Kode voucher dan promo spesial" />
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        <div className="relative flex-1 md:w-64">
                            ...
                            <input type="text" placeholder="Cari promo..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={search} onChange={e => setSearch(e.target.value)} />
                        </div>
                        <button onClick={handleOpenAdd} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-sm">
                            <Plus size={18}/> <span className="hidden md:inline">Promo Baru</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b text-xs uppercase text-gray-500 font-bold"><tr><th className="p-4">Nama Promo</th><th className="p-4">Kode</th><th className="p-4">Diskon</th><th className="p-4">Outlet</th><th className="p-4 text-right">Aksi</th></tr></thead>
                        <tbody className="divide-y divide-gray-100">
                            ...
                            filtered.map(i => (
                                <tr key={i.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4 font-mono"><span className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-100">{i.code}</span></td>
                                    <td className="p-4 text-sm">{i.discount_type === 'percent' ? i.discount_value + '%' : 'Rp ' + parseInt(i.discount_value).toLocaleString()}</td>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={formData.id ? "Edit Promo" : "Tambah Promo"}>
