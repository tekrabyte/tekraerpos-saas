...
export default function Modifiers() {
    ...
    const loadData = async () => {
        ...
        try {
            const [resData, resOutlets] = await Promise.all([
                api.get('/tenant/data/modifiers', { params: { outlet_id: filterOutlet !== 'all' ? filterOutlet : '' } }),
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.id) await api.put(`/tenant/data/modifiers/${formData.id}`, payload);
            else await api.post('/tenant/data/modifiers', payload);
            setShowModal(false); loadData();
        } catch (e) { alert("Gagal menyimpan."); }
    };

    const handleDelete = async (id) => {
        if (confirm("Hapus modifier?")) { await api.delete(`/tenant/data/modifiers/${id}`); loadData(); }
    };
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            <PageHeader title="Modifiers" subtitle="Tambahan opsi produk (Topping, Level, dll)" />
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        <div className="relative flex-1 md:w-64">
                            ...
                            <input type="text" placeholder="Cari modifier..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={search} onChange={e => setSearch(e.target.value)} />
                        </div>
                        <button onClick={handleOpenAdd} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-sm">
                            <Plus size={18}/> <span className="hidden md:inline">Modifier Baru</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b text-xs uppercase text-gray-500 font-bold"><tr><th className="p-4">Nama</th><th className="p-4">Tipe</th><th className="p-4 text-right">Harga</th><th className="p-4">Outlet</th><th className="p-4 text-right">Aksi</th></tr></thead>
                        <tbody className="divide-y divide-gray-100">
                            ...
                            filtered.map(i => (
                                <tr key={i.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4 text-sm"><span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${i.type==='add'?'bg-green-100 text-green-700':'bg-orange-100 text-orange-700'}`}>{i.type}</span></td>
                                    <td className="p-4 text-right font-mono font-medium">Rp {parseInt(i.price_adjustment).toLocaleString()}</td>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={formData.id ? "Edit Modifier" : "Tambah Modifier"}>
