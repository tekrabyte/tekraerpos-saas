...
export default function Taxes() {
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            <PageHeader title="Taxes (Pajak)" subtitle="Aturan pajak (PB1/PPN)" />
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        <div className="relative flex-1 md:w-64">
                            ...
                            <input type="text" placeholder="Cari pajak..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={search} onChange={e => setSearch(e.target.value)} />
                        </div>
                        <button onClick={handleOpenAdd} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-sm">
                            <Plus size={18}/> <span className="hidden md:inline">Pajak Baru</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b text-xs uppercase text-gray-500 font-bold"><tr><th className="p-4">Nama Pajak</th><th className="p-4">Tarif (%)</th><th className="p-4">Outlet</th><th className="p-4">Status</th><th className="p-4 text-right">Aksi</th></tr></thead>
                        <tbody className="divide-y divide-gray-100">
                            ...
                            filtered.map(i => (
                                <tr key={i.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4"><span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${i.is_active == 1 ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{i.is_active == 1 ? 'Aktif' : 'Nonaktif'}</span></td>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={formData.id ? "Edit Pajak" : "Tambah Pajak"}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={formData.is_active == 1} onChange={e => setFormData({...formData, is_active: e.target.checked ? 1 : 0})} className="rounded text-blue-600"/> Aktifkan Pajak Ini</label></div>
