...
export default function Taxes() {
    ...
    const loadData = async () => {
        try {
            const [resData, resOutlets] = await Promise.all([
                api.get('/tenant/data/taxes', { params: { outlet_id: filterOutlet !== 'all' ? filterOutlet : '' } }),
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.id) await api.put(`/tenant/data/taxes/${formData.id}`, payload);
            else await api.post('/tenant/data/taxes', payload);
            setShowModal(false); loadData();
        } catch (e) { alert("Gagal menyimpan."); }
    };

    const handleDelete = async (id) => {
        if (confirm("Hapus pajak?")) { await api.delete(`/tenant/data/taxes/${id}`); loadData(); }
    };

    const handleOpenAdd = () => {
        setFormData({ is_active: 1, outlet_id: filterOutlet !== 'all' ? filterOutlet : outlets[0]?.id });
        setShowModal(true);
    };
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            <div className="bg-white border-b px-6 py-4 flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-800">Taxes (Pajak)</h1>
                <button onClick={handleOpenAdd} className="bg-blue-600 text-white px-4 py-2 rounded font-bold flex items-center gap-2 hover:bg-blue-700"><Plus size={18}/> Tambah Pajak</button>
            </div>
            <div className="bg-gray-50 border-b px-6 py-3 flex gap-3">
                ...
                <div className="relative flex-1"><Search className="absolute left-3 top-2.5 text-gray-400" size={18} />
                    <input type="text" placeholder="Cari pajak..." className="w-full pl-10 pr-4 py-2 border rounded outline-none" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
            </div>
            <div className="flex-1 p-6 overflow-auto"><div className="bg-white rounded shadow border overflow-hidden"><table className="w-full text-left"><thead className="bg-gray-50 border-b text-xs uppercase text-gray-500 font-bold"><tr><th className="p-4">Nama Pajak</th><th className="p-4">Tarif (%)</th><th className="p-4">Outlet</th><th className="p-4">Status</th><th className="p-4 text-right">Aksi</th></tr></thead><tbody className="divide-y">
                {filtered.map(i => (
                    <tr key={i.id} className="hover:bg-blue-50"><td className="p-4 font-bold">{i.name}</td><td className="p-4 font-mono">{i.rate}%</td><td className="p-4 text-sm">{outlets.find(o=>o.id == i.outlet_id)?.name || '-'}</td><td className="p-4"><span className={`px-2 py-1 rounded text-xs font-bold ${i.is_active == 1 ? 'bg-green-100 text-green-800' : 'bg-gray-100'}`}>{i.is_active == 1 ? 'Aktif' : 'Nonaktif'}</span></td><td className="p-4 text-right flex justify-end gap-2"><button onClick={() => { setFormData(i); setShowModal(true); }} className="text-blue-600 p-1"><Edit size={16}/></button><button onClick={() => handleDelete(i.id)} className="text-red-600 p-1"><Trash2 size={16}/></button></td></tr>
                ))}
            </tbody></table></div></div>
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Form Pajak">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-4"><label className="flex items-center gap-2"><input type="checkbox" checked={formData.is_active == 1} onChange={e => setFormData({...formData, is_active: e.target.checked ? 1 : 0})} /><span className="text-sm">Aktifkan Pajak Ini</span></label></div>
