...
export default function SalesType() {
    ...
    const loadData = async () => {
        ...
        try {
            const [resData, resOutlets] = await Promise.all([
                api.get('/tenant/data/sales_types', { params: { outlet_id: filterOutlet !== 'all' ? filterOutlet : '' } }),
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.id) await api.put(`/tenant/data/sales_types/${formData.id}`, payload);
            else await api.post('/tenant/data/sales_types', payload);
            setShowModal(false); loadData();
        } catch (e) { alert("Gagal menyimpan."); }
    };

    const handleDelete = async (id) => {
        if (confirm("Hapus tipe penjualan?")) { await api.delete(`/tenant/data/sales_types/${id}`); loadData(); }
    };
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            <PageHeader title="Tipe Penjualan" subtitle="Atur jenis order (Dine in, Takeaway, dll)" />
                ...
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        <div className="relative flex-1 md:w-64">
                            ...
                            <input type="text" placeholder="Cari tipe..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={search} onChange={e => setSearch(e.target.value)} />
                        </div>
                        <button onClick={handleOpenAdd} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-sm">
                            <Plus size={18}/> <span className="hidden md:inline">Tipe Baru</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b text-xs uppercase text-gray-500 font-bold"><tr><th className="p-4">Nama Tipe</th><th className="p-4">Deskripsi</th><th className="p-4">Outlet</th><th className="p-4 text-right">Aksi</th></tr></thead>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={formData.id ? "Edit Sales Type" : "Tambah Sales Type"}>
